...
int main(int argc, char** argv)
{
    ...
    try
    {
        ...
        compileOptions.packMatricesInRowMajor = true;
        compileOptions.removeUnusedGlobals = true;
        compileOptions.optimizationLevel = 3;
        if (inputFileName == "FxaaPS.dsf")
            compileOptions.optimizationLevel = 0;
        compileOptions.shaderModel = { 6, 5 };
        ...
        for (size_t outputIdx = 0; outputIdx != targetName.size(); outputIdx++)
        {
            ...
            if (targetDesc[outputIdx].language == ShadingLanguage::SpirV)
            {
                compileOptions.numDXCArgs = 1;
                const char* dxcArgs[] = { "-fspv-target-env=vulkan1.3" };
                compileOptions.DXCArgs = dxcArgs;
            }
        }
