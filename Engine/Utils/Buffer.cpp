...
#define MEM_DEFAULT_ALIGN   16
    ...
    bool Buffer::Alloc(size_t size)
    {
        ...
        this->m_iSize = size;
        this->m_iBufSize = aether_alignup(size, MEM_DEFAULT_ALIGN); // sometimes we may access overflow(like simd), so align size to meet these requirements
    ...
    bool Buffer::Expand(size_t size)
    {
        if (m_bExternalMemory)
            return false;
        size_t aligned_size = aether_alignup(size, MEM_DEFAULT_ALIGN);
    ...
    void BitmapBuffer::UpdateSize(uint32_t width, uint32_t height, PixelFormat format, uint32_t depth)
    {
        ...
        this->m_iRowPitch = aether_alignup(width, MEM_DEFAULT_ALIGN) * Formatutil::NumComponentBytes(format);
        this->m_iSlicePitch = this->m_iRowPitch * this->m_iHeight;
