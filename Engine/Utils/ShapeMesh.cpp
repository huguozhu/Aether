...
namespace Aether
{
    namespace ShapeMesh
    {
        ...
        void CreateCube(MeshData& mesh_data)
        {
            ...
            mesh_data.eTopoType = EMeshTopologyType::Triangles;
        ...
        void CreateSphere(MeshData& mesh_data, float radius, uint32_t levels, uint32_t slices)
        {
        ...
        void CreateCone(MeshData& mesh_data, float radius, float height, uint32_t slices)
        {
        ...
        void CreateTerrain(MeshData& mesh_data, float width, float height, uint32_t slices_x, uint32_t slices_z, float max_texcoord_u, float max_texcoord_v,
            const std::function<float(float, float)>& heightFunc,
            const std::function<float3(float, float)>& normalFunc)
        {
        ...
        void CreateSprite2D(MeshData& mesh_data, float w, float h)
        {
            float half_w = w * 0.5f;
            float half_h = h * 0.5f;
            mesh_data.positions.push_back(float3(-half_w, -half_h, 0));
            mesh_data.positions.push_back(float3(+half_w, -half_h, 0));
            mesh_data.positions.push_back(float3(-half_w, +half_h, 0));
            mesh_data.positions.push_back(float3(+half_w, +half_h, 0));

            mesh_data.texcoords.push_back(float2(1, 1));
            mesh_data.texcoords.push_back(float2(0, 1));
            mesh_data.texcoords.push_back(float2(1, 0));
            mesh_data.texcoords.push_back(float2(0, 0));

            mesh_data.indices.push_back(0);
            mesh_data.indices.push_back(1);
            mesh_data.indices.push_back(2);
            mesh_data.indices.push_back(3);

            mesh_data.eTopoType = EMeshTopologyType::Triangle_Strip;
        ...
        {
            RHIContext& rc = engine->RHIContextInstance();
            RHIMeshPtr pMesh = rc.CreateMesh();            

            RHIBufferPtr buf_pos = rc.CreateVertexBuffer(sizeof(float3) * mesh_data.positions.size(), (const void*)&mesh_data.positions[0] );
            VertexStream vs_pos = { buf_pos, 0, sizeof(float3), {{0, 0, 1, VertexFormat::Float3, EVertexElementUsage::Position, false, 0}}, false };
            pMesh->AddVertexStream(vs_pos);

            if (mesh_data.texcoords.size() > 0)
            {
                RHIBufferPtr buf_tc = rc.CreateVertexBuffer(sizeof(float2) * mesh_data.texcoords.size(), (const void*)&mesh_data.texcoords[0]);
                VertexStream vs_tc = { buf_tc, 0, sizeof(float2), {{0, 0, 1, VertexFormat::Float2, EVertexElementUsage::TexCoord, false, 0}}, false };
                pMesh->AddVertexStream(vs_tc);
            }

            if (mesh_data.normals.size() > 0)
            {
                RHIBufferPtr buf_normal = rc.CreateVertexBuffer(sizeof(float3) * mesh_data.normals.size(), (const void*)&mesh_data.normals[0]);
                VertexStream vs_normal = { buf_normal, 0, sizeof(float3), {{0, 0, 1, VertexFormat::Float3, EVertexElementUsage::Normal, false, 0}}, false };
                pMesh->AddVertexStream(vs_normal);
            }

            if (mesh_data.tangent.size() > 0)
            {
                RHIBufferPtr buf_tangent = rc.CreateVertexBuffer(sizeof(float4) * mesh_data.tangent.size(), (const void*)&mesh_data.tangent[0]);
                VertexStream vs_tangent = { buf_tangent, 0, sizeof(float4), {{0, 0, 1, VertexFormat::Float4, EVertexElementUsage::Tangent, false, 0}}, false };
                pMesh->AddVertexStream(vs_tangent);
            }

            if (mesh_data.indices.size() > 0)
            {
                RHIBufferPtr buf_index = rc.CreateIndexBuffer(sizeof(uint16_t) * mesh_data.indices.size(), (const void*)&mesh_data.indices[0]);
                pMesh->SetIndexBuffer(buf_index, EIndexBufferType::UInt16);
            }
