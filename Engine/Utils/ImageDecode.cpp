...
namespace Aether
{
    ...
    BitmapBufferPtr ImageDecode::Decode(uint8_t* data, size_t size, ImageType imageType /* = ImageType::UNKNOWN */)
    {
        switch (imageType)
        {
        case ImageType::JPEG:
    ...
    BitmapBufferPtr ImageDecode::DecodeFromFile(std::string const& filepath, ImageType imageType)
    {
        std::vector<uint8_t> fileData;
        AETHER_RET_NULL_IF_FAIL(read_file_content(filepath.c_str(), "rb", fileData));
        return Decode((uint8_t*)fileData.data(), fileData.size(), imageType);
    }
};
