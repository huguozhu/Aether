...
namespace Aether
{
    ...
    AResult TransformAnimationTrack::Run(float time)
    {
        return Run(time, JointFusionMode::Cover);
    }
    AResult TransformAnimationTrack::Run(float time, JointFusionMode jointFusionMode)
    {
    ...
    AResult TransformAnimationTrack::GetInterpolatedKeyFrame(float time, KeyFrame& kf)
    {
        return GetInterpolatedKeyFrame(time, kf, JointFusionMode::Cover);
    }
    AResult TransformAnimationTrack::GetInterpolatedKeyFrame(float time, KeyFrame& kf, JointFusionMode jointFusionMode)
    {
        uint32_t kfIdx;
        AETHER_RETIF_FAIL(GetKeyFrameIdxByTime(time, kfIdx));
    ...
    AResult MorphTargetAnimationTrack::Run(float time)
    {
        return Run(time, MorphTargetWeightFusionMode::Cover);
    }
    AResult MorphTargetAnimationTrack::Run(float time, MorphTargetWeightFusionMode weightFusionMode)
    {
        /*if (!m_pSceneComponent)
        {
    ...
    AResult MorphTargetAnimationTrack::GetInterpolatedKeyFrame(float time, KeyFrame& kf)
    {
        uint32_t kfIdx;
        AETHER_RETIF_FAIL(GetKeyFrameIdxByTime(time, kfIdx));
