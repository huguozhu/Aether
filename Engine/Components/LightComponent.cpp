...
namespace Aether
{
     ...
     * Light
     ******************************************************************************/
    LightComponent::LightComponent(AetherEngine* engine, LightType type, std::string const& name /* = "LightComponent" */)
        : SceneComponent(engine, name, ComponentType::Light), m_eLightType(type)
    {
        switch (type)
        {
        case LightType::Num:
    ...
    LightComponentPtr LightComponent::CreateLightComponent(AetherEngine* engine, LightType lightType, std::string const& name)
    {
        if (engine == nullptr || uint32_t(lightType) >= uint32_t(LightType::Num) || lightType == LightType::Unknown)
        {
        ...
        case LightType::Ambient:
            pLightComponent = std::static_pointer_cast<LightComponent>(MakeSharedPtr<AmbientLightComponent>(engine));
            break;
        case LightType::Directional:
            pLightComponent = std::static_pointer_cast<LightComponent>(MakeSharedPtr<DirectionalLightComponent>(engine));
            break;
        case LightType::Spot:
            pLightComponent = std::static_pointer_cast<LightComponent>(MakeSharedPtr<SpotLightComponent>(engine));
            break;
        case LightType::Point:
            pLightComponent = std::static_pointer_cast<LightComponent>(MakeSharedPtr<PointLightComponent>(engine));
            break;
     ...
     ******************************************************************************/
    AmbientLightComponent::AmbientLightComponent(AetherEngine* engine, std::string const& name)
        : LightComponent(engine, LightType::Ambient, name)
    {
    }
     ...
     ******************************************************************************/
    DirectionalLightComponent::DirectionalLightComponent(AetherEngine* engine, std::string const& name)
        : LightComponent(engine, LightType::Directional, name)
    {
        m_pSMCamera = MakeSharedPtr<LightCameraComponent>(engine);
    ...
    void DirectionalLightComponent::CascadedShadow(bool v)
    {
        ...
        //    m_pCsmCamera = MakeSharedPtr<CsmCameraComponent>(engine);
     ...
     * SpotLightComponent
     ******************************************************************************/
    SpotLightComponent::SpotLightComponent(AetherEngine* engine, std::string const& name)
        : LightComponent(engine, LightType::Spot, name)
    {
        m_pSMCamera = MakeSharedPtr<LightCameraComponent>(engine);
    ...
    PointLightComponent::PointLightComponent(AetherEngine* engine, std::string const& name)
        : LightComponent(engine, LightType::Point, name)
    {
        m_fShadowBias = 0.05;
        Matrix4 proj = Math::PerspectiveLH(90.0 * Math::DEG2RAD, 1.0, DEFAULT_SM_CAMERA_NEAR, DEFAULT_SM_CAMERA_FAR);
        for (uint32_t face = (uint32_t)ECubeFaceType::Positive_X; face < (uint32_t)ECubeFaceType::Num; face++)
        {
            m_aSMCameras[face] = MakeSharedPtr<LightCameraComponent>(engine);
