...
export namespace Aether
{
    ...
    {
    public:
        AnimationComponent(AetherEngine* engine);
        virtual ~AnimationComponent() {}

        virtual AResult             Tick(float delta_time) override;
        AResult                     SectionTick(float delta_time, uint32_t sectionIdx);
    ...
    public:
        ...
        virtual AResult   Run(float time) = 0;
    ...
    protected:
        ...
        virtual AResult   GetInterpolatedKeyFrame(float time, KeyFrame& kf) = 0;
        AResult           GetKeyFrameIdxByTime(float& time, uint32_t& kfIdx);
