...
#include <windows.h>
#include <d3d12.h>
module Aether:D3D12GpuMemoryAllocator;
...
namespace Aether
{
	uint32_t ConstantDataAligment = D3D12_CONSTANT_BUFFER_DATA_PLACEMENT_ALIGNMENT;
	uint32_t StructuredDataAligment = D3D12_RAW_UAV_SRV_BYTE_ALIGNMENT;
	uint32_t TextureDataAligment = D3D12_TEXTURE_DATA_PLACEMENT_ALIGNMENT;
	...
	* D3D12GpuMemoryPage
	*******************************************************************************/
	D3D12GpuMemoryPage::D3D12GpuMemoryPage(AetherEngine* engine, bool is_upload, uint32_t size_in_bytes)
		:m_pEngine(engine), m_bIsUpLoad(is_upload)
	{
		...
		ThrowIfFailed(m_pResource->Map(0, &read_range, &m_pCpuAddr));
	...
	D3D12GpuMemoryPage::D3D12GpuMemoryPage(D3D12GpuMemoryPage&& rhs)
		: m_pEngine(rhs.m_pEngine), m_bIsUpLoad(rhs.m_bIsUpLoad), m_pResource(std::move(rhs.m_pResource)), m_pCpuAddr(std::move(rhs.m_pCpuAddr)),
	...
	*******************************************************************************/
	D3D12GpuMemoryAllocator::D3D12GpuMemoryAllocator(AetherEngine* engine, bool is_upload)
		: m_pEngine(engine), m_bIsUpload(is_upload)
	{
	}
	D3D12GpuMemoryAllocator::D3D12GpuMemoryAllocator(D3D12GpuMemoryAllocator&& rhs)
		: m_pEngine(rhs.m_pEngine), m_bIsUpload(rhs.m_bIsUpload), m_vPages(std::move(rhs.m_vPages)), m_vLargePages(std::move(rhs.m_vLargePages))
	{
	}
	...
	{
		AETHER_ASSERT(alignment <= SegmentSize);
		...
		{
			auto& large_page = m_vLargePages.emplace_back(D3D12GpuMemoryPage(m_pEngine, m_bIsUpload, aligned_size));
		...
		D3D12GpuMemoryPage new_page(m_pEngine, m_bIsUpload, PageSize);
		...
		[[maybe_unused]] std::lock_guard<std::mutex>& proof_of_lock, D3D12GpuMemoryBlock& mem_block, uint64_t fence_value)
	{
		if (mem_block.GetSize() <= PageSize)
		{
			...
			LOG_ERROR("This memory block is not allocated by this allocator");
		}
	}
