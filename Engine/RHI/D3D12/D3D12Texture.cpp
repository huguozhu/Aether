...
namespace Aether
{
    ...
    * D3D12Texture
    *******************************************************************************/
    D3D12Texture::D3D12Texture(AetherEngine* engine, const RHITexture::Desc& tex_desc)
        :RHITexture(tex_desc), D3D12Resource(engine)
    {
        m_eDxgiFormat = D3D12Translate::TranslateToPlatformFormat(tex_desc.format);
    ...
    {
        AETHER_ASSERT(m_desc.flags & RESOURCE_FLAG_GPU_READ);
        ...
        else
        {
            ...
            D3D12SrvPtr pSrv = MakeSharedPtr<D3D12Srv>(m_pEngine, this, desc);
            return m_mD3dSrvs.emplace(hash_val, pSrv).first->second;
        }
    }
    D3D12SrvPtr const& D3D12Texture::GetD3DSrv(uint32_t array_index, ECubeFaceType face, uint32_t first_level, uint32_t num_levels)
    {
        AETHER_ASSERT(m_desc.flags & RESOURCE_FLAG_GPU_READ);
    ...
    {
        AETHER_ASSERT(m_desc.flags & RESOURCE_FLAG_GPU_WRITE);
        AETHER_ASSERT(first_array_index < m_desc.num_array);
        AETHER_ASSERT(first_array_index + array_size <= m_desc.num_array);
        ...
        else
        {
            ...
            D3D12RtvPtr pRtv = MakeSharedPtr<D3D12Rtv>(m_pEngine, this, desc);
    ...
    D3D12RtvPtr const& D3D12Texture::GetD3DRtv(uint32_t array_index, ECubeFaceType face, uint32_t mip_level)
    {
        AETHER_ASSERT(m_desc.flags & RESOURCE_FLAG_GPU_WRITE);
    ...
    D3D12DsvPtr const& D3D12Texture::GetD3DDsv(uint32_t first_array_index, uint32_t array_size, uint32_t mip_level)
    {
        ...
        else
        {
            ...
            D3D12DsvPtr pDsv = MakeSharedPtr<D3D12Dsv>(m_pEngine, this, desc);
    ...
    D3D12DsvPtr const& D3D12Texture::GetD3DDsv(uint32_t array_index, ECubeFaceType face, uint32_t mip_level)
    {
        AETHER_ASSERT(m_desc.flags & RESOURCE_FLAG_GPU_WRITE);
    ...
    {
        AETHER_ASSERT(m_desc.flags & RESOURCE_FLAG_UAV);
        ...
        else
        {
            ...
            D3D12UavPtr pUav = MakeSharedPtr<D3D12Uav>(m_pEngine, this, desc);
    ...
    D3D12UavPtr const& D3D12Texture::GetD3DUav(uint32_t first_array_index, uint32_t array_size, ECubeFaceType first_face, uint32_t num_faces, uint32_t mip_level)
    {
        AETHER_ASSERT(m_desc.flags & RESOURCE_FLAG_UAV);
    ...
    AResult D3D12Texture::DoCreate(D3D12_RESOURCE_DIMENSION dim, uint32_t width, uint32_t height, uint32_t depth,
        uint32_t array_size, std::span<BitmapBufferPtr> const& bitmap_datas)
    {
        ...
        if (!bitmap_datas.empty())
        {
            ...
            D3D12GpuMemoryBlock upload_mem_block =
                rc.AllocUploadMemBlock(static_cast<uint32_t>(required_size), TextureDataAligment);
    ...
    * D3D12Texture2D
    *******************************************************************************/
    D3D12Texture2D::D3D12Texture2D(AetherEngine* engine, const RHITexture::Desc& tex_desc)
        :D3D12Texture(engine, tex_desc)
    {
        m_vCurrStates.assign(m_desc.num_array * m_desc.num_mips, D3D12_RESOURCE_STATE_COMMON);
    }
    D3D12Texture2D::D3D12Texture2D(AetherEngine* engine, ID3D12ResourcePtr const& d3d_tex)
        :D3D12Texture(engine, RHITexture::Desc{})
    {
        D3D12_HEAP_PROPERTIES heap_prop;
        ...
        m_desc.format = D3D12Translate::TranslateFromPlatformFormat(desc.Format);
    ...
    AResult D3D12Texture2D::Create(std::span<BitmapBufferPtr> const& bitmap_datas)
    {
        return this->DoCreate(D3D12_RESOURCE_DIMENSION_TEXTURE2D, m_desc.width, m_desc.height, m_desc.depth, 1, bitmap_datas);
    }
    AResult D3D12Texture2D::Update(std::span<BitmapBufferPtr> const& bitmap_datas)
    {
        return A_Success;
    }
    AResult D3D12Texture2D::Resolve()
    {
        return A_Success;
    }
    AResult D3D12Texture2D::DumpSubResource2D(BitmapBufferPtr bitmap_data, uint32_t array_index, uint32_t mip_level, Rect<uint32_t>* rect)
    {
        return A_Success;
    }
    ...
    * D3D12TextureCube
    *******************************************************************************/
    D3D12TextureCube::D3D12TextureCube(AetherEngine* engine, const RHITexture::Desc& tex_desc)
        :D3D12Texture(engine, tex_desc)
    {
        m_vCurrStates.assign(6 * m_desc.num_array * m_desc.num_mips, D3D12_RESOURCE_STATE_COMMON);
    }

    AResult D3D12TextureCube::Create(std::span<BitmapBufferPtr> const& bitmap_datas)
    {
        return this->DoCreate(D3D12_RESOURCE_DIMENSION_TEXTURE2D, m_desc.width, m_desc.height, 1, m_desc.num_array * 6, bitmap_datas);
    }
    AResult D3D12TextureCube::Update(std::span<BitmapBufferPtr> const& bitmap_datas)
    {
        return A_Success;
    }
    AResult D3D12TextureCube::DumpSubResourceCube(BitmapBufferPtr bitmap_data, ECubeFaceType face, uint32_t array_index, uint32_t mip_level, Rect<uint32_t>* rect)
    {
        return A_Success;
    }
    ...
    void D3D12TextureCube::FillSrvDesc(D3D12_SHADER_RESOURCE_VIEW_DESC& desc, uint32_t array_index, ECubeFaceType face, uint32_t first_level, uint32_t num_levels)
    {
        switch (m_desc.format)
        {
        case PixelFormat::D16:      desc.Format = DXGI_FORMAT_R16_UNORM;                break;
    ...
    void D3D12TextureCube::FillRtvDesc(D3D12_RENDER_TARGET_VIEW_DESC& desc, uint32_t array_index, ECubeFaceType face, uint32_t mip_level)
    {
        switch (m_desc.format)
        {
        case PixelFormat::D16:      desc.Format = DXGI_FORMAT_R16_UNORM;                break;
    ...
    void D3D12TextureCube::FillDsvDesc(D3D12_DEPTH_STENCIL_VIEW_DESC& desc, uint32_t array_index, ECubeFaceType face, uint32_t mip_level)
    {
        switch (m_desc.format)
        {
        case PixelFormat::D16:      desc.Format = DXGI_FORMAT_D16_UNORM;                break;
    ...
    void D3D12TextureCube::FillUavDesc(D3D12_UNORDERED_ACCESS_VIEW_DESC& desc, uint32_t first_array_index, uint32_t array_size, uint32_t mip_level)
    {
        ...
        return this->FillUavDesc(desc, first_array_index, array_size, ECubeFaceType::Positive_X, 6, mip_level);
    }
    void D3D12TextureCube::FillUavDesc(D3D12_UNORDERED_ACCESS_VIEW_DESC& desc, uint32_t first_array_index, uint32_t array_size, ECubeFaceType first_face, uint32_t num_faces, uint32_t mip_level)
    {
        switch (m_desc.format)
        {
        case PixelFormat::D16:      desc.Format = DXGI_FORMAT_R16_UNORM;                break;
    ...
    * D3D12Texture3D
    *******************************************************************************/
    D3D12Texture3D::D3D12Texture3D(AetherEngine* engine, const RHITexture::Desc& tex_desc)
        :D3D12Texture(engine, tex_desc)
    {
        m_vCurrStates.assign(m_desc.num_array * m_desc.num_mips, D3D12_RESOURCE_STATE_COMMON);
    }
    AResult D3D12Texture3D::Create(std::span<BitmapBufferPtr> const& bitmap_datas)
    {
    ...
    AResult D3D12Texture3D::Update(std::span<BitmapBufferPtr> const& bitmap_datas)
    {
        return A_Success;
    }
    AResult D3D12Texture3D::DumpSubResource3D(BitmapBufferPtr bitmap_data, uint32_t array_index, uint32_t mip_level, Box<uint32_t>* box)
    {
        return A_Success;
    }
