...
namespace Aether
{
    SceneManager::SceneManager(AetherEngine* engine)
        :m_pEngine(engine)
    {
        m_pRootComponent = MakeSharedPtr<SceneComponent>(engine);
    ...
    AResult SceneManager::Tick(float delta_time)
    {
        ...
        if (m_bSceneDirty)
        {
            ...
            if (m_vLightList.empty())
            {
                ...
                if (m_vDefaultLights.empty())
                {
                    ...
                    m_vDefaultLights[0] = MakeSharedPtr<AmbientLightComponent>(m_pEngine, "defualt light 0");
                    m_vDefaultLights[1] = MakeSharedPtr<DirectionalLightComponent>(m_pEngine, "defualt light 1");
                    m_vDefaultLights[2] = MakeSharedPtr<DirectionalLightComponent>(m_pEngine, "defualt light 2");
        ...
        for (Entity* entity : m_vCurEntities)
        {
            ...
            for (ComponentPtr component : comps)
            {
                if (component->GetComponentType() == ComponentType::Animation)
                    AETHER_RETIF_FAIL(component->Tick(delta_time));
            }
        }
        for (Entity* entity : m_vCurEntities)
        {
        ...
        {
            //AETHER_RETIF_FAIL(sprite->Tick(delta_time));
        }
    ...
    //RHIBufferPtr& SceneManager::GetLightInfoCBuffer()
    //{
    //    if (!m_LightInfoCBuffer)
    //    {
    //        m_LightInfoCBuffer = m_pEngine->RHIContextInstance().CreateConstantBuffer(sizeof(LightInfo) * MAX_LIGHT, RESOURCE_FLAG_CPU_WRITE);
    //    }
    ...
    //        float exposure = 1.0;
    //        if (m_pEngine->GetLightingMode() == LightingMode::PBR)
    //        {
    ...
    //        }
    //        if (info.castShadow)
    //            info.shadowMapIndex = m_pEngine->SceneRendererInstance().GetShadowLayer()->GetShadowMapIndexByLightIndex(lightIndex);
    ...
    //    {
    //        m_ViewInfoCBuffer = m_pEngine->RHIContextInstance().CreateConstantBuffer(sizeof(CameraInfo), RESOURCE_FLAG_CPU_WRITE);
    //    }
    ...
    //}
    //AResult SceneManager::RenderSprite2DScene()
    //{
    //    for (auto& component : m_vSprite2DComponentList)
    //    {
    //        AETHER_RETIF_FAIL(component->Render());
    //    }
    //    return A_Success;
    //}
    ...
    //    else
    //    {
    //        CameraComponentPtr pCam = MakeSharedPtr<CameraComponent>(m_pEngine, CameraProjectionType::Orthographic);
